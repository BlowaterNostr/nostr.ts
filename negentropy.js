...
export class Negentropy {
    ...
    addItem(timestamp, id) {
        ...
        if (id.byteLength > 64) throw Error("bad length for id");
        if (id.byteLength < this.idSize) throw Error(`bad length for id: ${id.byteLength}`);
            ...
            if (mode === 0) { // Skip
                ...
                if (this.isInitiator) {
                    ...
                    let flushIdListOutput = () => {
                        ...
                        let nextSplitBound = (it + 1) >= upper
                            ? currBound
                            : this.getMinimalBound(this.items[it], this.items[it + 1]);
    ...
    splitRange(lower, upper, lowerBound, upperBound, outputs) {
        ...
        if (numElems < buckets * 2) {
            ...
            for (let i = 0; i < buckets; i++) {
                let ourXorSet = new Uint8Array(this.idSize);
                for (
                    let bucketEnd = curr + itemsPerBucket + (i < bucketsWithExtra ? 1 : 0);
                    curr != bucketEnd;
                    curr++
                ) {
                ...
                outputs.push({
                    ...
                    end: curr === this.items.length
                        ? upperBound
                        : this.getMinimalBound(this.items[curr - 1], this.items[curr]),
                    payload: payload,
                });
    ...
    buildOutput() {
        ...
        if (
            (!this.isInitiator && this.pendingOutputs.length) ||
            (this.isInitiator && output.length == 0 && this.continuationNeeded)
        ) {
