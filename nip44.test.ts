...
import { assertEquals, assertMatch, fail } from "https://deno.land/std@0.176.0/testing/asserts.ts";
const v2vec = vec.v2;
...
Deno.test("decrypt", async () => {
    for (const v of v2vec.invalid.decrypt) {
        const err = v2.decrypt(v.payload, hexToBytes(v.conversation_key)) as Error;
...
Deno.test("get_conversation_key", async () => {
    for (const v of v2vec.invalid.get_conversation_key) {
        const err = v2.utils.getConversationKey(v.sec1, v.pub2) as Error;
